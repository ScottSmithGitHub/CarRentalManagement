...
namespace CarRentalManagement.Server.Repository
{
    public class UnitOfWork : IUnitOfWork
    {
        ...
        private IGenericRepository<Color> _colours;
        ...
        public IGenericRepository<Color> Colors
            => _colours ??= new GenericRepository<Color>(_context);
        ...
        public async Task Save(HttpContext httpContext)
        {
            ...
            var entries = _context.ChangeTracker.Entries().Where(q => q.State == EntityState.Modified ||
            q.State == EntityState.Added);

            foreach (var entry in entries)
            {
                ((BaseDomainModel)entry.Entity).DateUpdated = DateTime.Now;
                ((BaseDomainModel)entry.Entity).UpdatedBy = "Scott"; // user;

                if (entry.State == EntityState.Added)
                {
                    ((BaseDomainModel)entry.Entity).DateCreated = DateTime.Now;
                    ((BaseDomainModel)entry.Entity).CreatedBy = "Scott"; // user;
                }
            }
